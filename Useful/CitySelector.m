...
%are related to pool cities (cities in database)(The two names, pool and database, are interchangable here).
...
for i=1:l
    ...
    %If not found, attempt to get info from the Internet databases: Data Science
...
for j=1:L
    ...
    %Calculate traveling distance for each meeting participant to j-th city
    %in the pool.
    dis=zeros(1,l);
    for k=1:l
        dis(k)=deg2km(distance(peopCoor(k,1),peopCoor(k,2),lat,lon)); %%%%%%%
    end
    %Calculate air fare. For details of the following codelines, please refer to the paper.
    ...
    t_rest=round(t_rest);
    %Mark '(x)' after name of cities in the pool cannot reached undr the crrent budget.
    if t_rest<0
